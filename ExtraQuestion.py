...
def gradientCheck(X, y, weights, lambda_reg, epsilon):
    numerical_gradients = []
    backprop_gradients = backwardPropagation(X, y, weights, lambda_reg)
    ...
    for l, theta in enumerate(weights):
        ...
        for i in range(theta.shape[0]):
            for j in range(theta.shape[1]):
                ...
                J_plus = calculateCost(X, y, weights_plus, lambda_reg)
                J_minus = calculateCost(X, y, weights_minus, lambda_reg)
...
def testCase(case):
    ...
    for epsilon in epsilon_values:
        ...
        gradientCheck(X_train, y_train, weights_list, lambda_reg, epsilon)
